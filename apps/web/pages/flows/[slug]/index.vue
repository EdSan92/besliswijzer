...
const { data: flow, error } = await useAsyncData(`flow-${slug}-${versionQuery ?? 'latest'}`, () =>
  $fetch<PublicFlowResponse>(`${apiBase}/api/v1/public/flows/${slug}${query}`),
)

if (error.value || !flow.value) {
